...
<xsl:template match="nir:atto | nir:def | nir:soggetto | nir:nome |	nir:data | nir:ente | nir:importo | nir:luogo">
    <xsl:element name="span" use-attribute-sets="XsltMapperSetClass">
    	<xsl:apply-templates select="mapper:getTextNodeIfEmpty(.)" />
        <xsl:apply-templates />
    </xsl:element>
