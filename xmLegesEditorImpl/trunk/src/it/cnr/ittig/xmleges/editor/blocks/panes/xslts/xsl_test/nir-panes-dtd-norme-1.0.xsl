...
<!-- VARIANTE 1 in questo modo non funziona "use-attribute-sets"-->
<!--xsl:template match="nir:emanante">
	<div class="title">
		<xsl:apply-templates select="mapper:getTextNodeIfEmpty(.)" />
		<xsl:apply-templates />
...
<!-- VARIANTE 2;  in questo modo non posso aggiungere l'attributo "class" -->
<!-- ok: vedi nir-generico-style.css -->
<xsl:template match="nir:emanante">
	<xsl:element name="div"  use-attribute-sets="XsltMapperSetClass">
