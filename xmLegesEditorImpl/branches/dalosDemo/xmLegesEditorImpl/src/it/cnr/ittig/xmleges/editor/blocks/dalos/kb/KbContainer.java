	...
	OntModel getModel(String type, String reasoner) {
		...
		if(reasoner.length() > 0) {
			...
			if(reasoner.equalsIgnoreCase("pellet")) {
				//puï¿½ servire pellet ? oppure un external reasoner?
			}
			if(r == null) {
...
//			om.read(KbConf.METALEVEL_PROP);
	...
	void addLexicalProperties(Synset syn) {		
		//Aggiunge le proprietï¿½ generiche e quelle lessicali
		...
		//Prendi in considerazione solo le proprietï¿½ che sono subProperty di language-property
		//(cosï¿½ aggiunge anche language-property come relazione, che mappa verso l'unione
		//dei synset mappati da tutte le proprietï¿½ lessicali... puï¿½ essere utile?)
...
//			}			
	...
	void addSemanticProperties(Synset syn) {
		//Aggiunge le proprietï¿½ semantiche
