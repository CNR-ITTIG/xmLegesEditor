...
public class CreaMultivigenteFormImpl implements CreaMultivigenteForm, Loggable, ActionListener, Serviceable, Initializable, FormClosedListener, FormVerifier {
	...
	public void actionPerformed(ActionEvent e) {
			...
			if (virgolettaDaInserire==null)
				if (virgolettaDaEliminare!=null)
					...
					if (partizione.indexOf("#")!=-1) {
						partizione = partizione.substring(1+partizione.indexOf("#"));  //xex  art1-com3-leta (significativo � let)
		...
		if (e.getSource() == file) {
			if ((attivo = apriFile())!=null) {
				...
				//effettuo un test per vedere se la norma aperta � quella che mi aspettavo
				try {
				if (!urnAttivo.equals(UtilDom.getAttributeValueAsString(UtilDom.getElementsByTagName(docAttivo,docAttivo,"originale")[0], "xlink:href"))) {
					errore.setText("Il file aperto non � corretto");
		...
		if (e.getSource() == scrivi) {
			...
			if (creaMetadato) {
				...
				//o se � una modifica, di un mod, gi� in precedenza modificato. 
	...
	private boolean proponiModificaTesto(Node partizioneIndicata) {
		...
		if (partizioneIndicata!=null)
			if (scrivi.isEnabled()) { //ho cambiato posizione e avevo gi� proposto qualcosa => cancella vecchia proposta
				...
				//Attenzione, se avevo pi� partizioni nella virgoletta => devo cancellare anche le altre integrazioni
			...
			else 		//se ho selezionato delle parole manualmente, mi fido anche se nella realt� avevo chiesto di selezionarne altre
				try {
...
//-----FASE1 (processo il Meta, creo Mod/TestoPrima)
		...
		if (tempNovella!=null) {
			...
			if (trovati.length>0)
				virgolettaDaInserire = UtilDom.getElementsByAttributeValue(docAttivo,docAttivo,"id",idVirgoletta)[0].cloneNode(true);  //lo clono perch� potrei integrarlo con num e rubrica e poi rimosizionarmi manualmente altrove
			else {
		...
		//test per capire se la modifica corrente � stata gi� applicata
				...
				for (int z=0; z<listaDegliEventi.size(); z++)
					if (((String)idEventiDaTestare.get(j)).equals(UtilDom.getAttributeValueAsString((Node)listaDegliEventi.get(z), "fonte"))) 
						if ((dataEvento).equals(UtilDom.getAttributeValueAsString((Node)listaDegliEventi.get(z), "data"))) {
							...
							if (partizioneIndicata==null) {	
							//per ora mi fermo qui. Ho verificato che esiste gi� un evento 'identico' nel modificato. Presumo quindi sia gi� stato modificato.
							//In realt� dovrei prelevare l'attributo ID. Prendere tutte le partizioni che hanno attributo INIZIOVIGORE=id e/o
							//FINEVIGORE=id (a seconda del tipo di modifica che voglio apportare), prelevare ID (della partizione/span) e ricercarlo
							//fra i meta delle disposizioni passive. A questo punto DECIDERE se � stata gi� applicata oppure no.
								 
								//NO. o questo test lo faccio solo sulla prima modifica di ogni norma modificante, o scendo nel testo.
								
							try {
							listModel.setElementAt(((String) listModel.get(getPosLista())) + " --> saltata, gi� applicata", getPosLista());
							} catch (Exception e) {}
							correnteEvento++;
							return true;
							}
		...
		if (posizione!=null) {
			if (parole && partizioneIndicata==null) {	//cerco parole/capoverso/periodo/alinea/rubrica...
				if ("parole".equals(tipoModifica)) {	
					//se � parole, cerco le occorrenze di parole
		...
		if (posizione!=null			//se � null NON PROPONGO LA MODIFICA (faccio vedere in ELSE il testo del MOD)
...
//-----FASE3 (TestoDopo/modifiche sul DOM)
			...
			//Se la virgoletta conteneva pi� partizioni (per ora ho inserito solo 1�figlio di virgoletta) creo automaticamente
...
//-----FASE4 (Creazione frammenti xTab)		
			...
			//Aggiungo... se la virgoletta conteneva pi� partizioni
			if (figliVirgoletta!=null)
				for (int i=1; i<figliVirgoletta.size(); i++)
	...
	private Node cerca_Rif_e_Bordo(Document doc, Node metaCorrente) {
		...
		tipoModifica = "";	//� ci� che voglio modificare
		/*
		<dsp:novella>
	...
	<dsp:norma xlink:href="urn:nir:stato:legge:1999;2" xlink:type="simple">
		...
		*/	
		...
		if (partizione.indexOf("#")!=-1) {
			...
			//Questo meccanismo cerca id esatto della partizione. Per come numera l'editor, questo non � possibile
...
//				trovataPosizione=false;
	...
	//posizionamentoManuale.aggiungiBordo(numOrd,tipo); 										BUTTATO TUTTO PER ORA
				...
				if (tag!=null) {   //Cerco il bordo
					...
					if (numOrd.equals("penultimo") || numOrd.equals("secondo"))
						prendi = 2;				
					if (id==null && !"".equals(numOrd)) //esempio: rubrica, alinea, coda, ... non hanno cardinalit� (� implicitamente 1)
					...
					for (int j=da; j<a; j++) {
						...
						if (prendi==0 && !"".equals(numOrd)) {	//faccio considerazioni sull'id
							...
							if (++nValidi>=prendi) {	// >= cos� se arrivo con 0 o 1 prendo comunque il primo
								posiz = cerco[indice];
								break;
							}
						}
					...
					if (tipo.equals("periodo") || tipo.equals("capoverso")) {  //capoverso o bordo vanno intesi come un periodo che finisce con .
						...
						while (st.hasMoreTokens()) {
							...
							else if (lastParola.length()>4)		//trovato 'parola.', con parola pi� lunga di 3 caratteri
		...
		//test se sono posizionato su intera norma, ma ho gi� un finevigore, deseleziono
	...
	private Node conta(Document doc, Node dove, String nomeTag, String numeroTag, boolean ordinale) {
		...
		//cerco il primo tag corrispondente in profondit�
		...
		if (posizioniTrovate.length>0) {
			...
			if (ordinale) {
				...
				if ("".equals(getNum(doc,dove))) {//NUM nulli
					//considero i meta (se � un testo rosso vale come uno nel conteggio solo se NON � una Sostituzione
					...
					for (int i=0; i<posizioniTrovate.length; i++)	//restituisco quello con NUM che inizia con numeroTag escludendo i testi Rossi 
						if (isTestoVigente(posizioniTrovate[i])) {
							...
							if (numero.startsWith(numeroTag) || ("".equals(numero) && conta<=0))   //se trovo uno senza num , che � almeno numeroTag�, allora lo restituisco
								return posizioniTrovate[i];		
						}
				}
			}
