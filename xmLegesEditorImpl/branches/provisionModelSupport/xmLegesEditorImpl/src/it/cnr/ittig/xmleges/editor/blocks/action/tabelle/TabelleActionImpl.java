...
public class TabelleActionImpl implements TabelleAction, Loggable, Serviceable, Configurable, Initializable, EventManagerListener {
	...
	public class CreaTabellaAction extends MyAbstractAction {
		...
		public boolean canDoAction(Node[] n) {
			...
			//Se non � selezionato nulla le funzioni canInsertTable non funzionano
			if (end==-1) 
	            end =1;
			if (start==-1) 
	            start =1;
	...
	public class EliminaRigaAction extends MyAbstractAction {
		//
		// Per elimina righe lascio la possibilit� della selezione multipla, quindi
		// non � pi� suff.controllare "canDeleteRiga" x singole righe, bisogna anche
		// considerare il numero di righe che andr� a cancellare e il tipo.
...
//				return false;
	...
	public class EliminaColonnaAction extends MyAbstractAction {
		public boolean canDoAction(Node[] n) {
			//
			// Per elimina colonna lascio la possibilit� della selezione multipla, quindi
			// non � pi� suff.controllare "canDeleteColonna" x singole colonne, ma bisogna
			// contare il numero di colonne che andr� a cancellare.
			//     (Al momento accantonato)
			//
